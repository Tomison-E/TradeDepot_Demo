...
import 'package:tradedepot_demo/app/signIn/presentation/logic/userVM.dart';
...
class _SignInFormState extends State<SignInForm> {
  ...
  Future<void> handleSubmitted() async {
    ...
    if (!form.validate()) {
      ...
      form.save();
      dialog(context);
      final test = await context.read(userVMProvider).signIn(email, password);
      test.maybeWhen(
        success: (bool data) async {
          Navigator.pop(context);
          if (data) {
            /*final user = await context.read(userVMProvider).getCurrentUser();
            user.maybeWhen(
                success: (data) async {
                ...
                orElse: () {
                  showInSnackBar('Unable to verify current user');
                }
            );*/
            _autoValidate = AutovalidateMode.disabled;
            reset(form);
